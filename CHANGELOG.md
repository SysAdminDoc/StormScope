...
- Added an optional, default-off NOAA NDBC marine-buoy layer with viewport/time-bounded wind, wave-period, wave-height, and sea-surface-temperature observations, DOM-only station popups, freshness/attribution, safe JSONP transport, last-good recovery, scene persistence, English/Spanish copy, and headless coverage.
- Added an optional, default-off NOAA CPC outlook layer with viewport-bounded monthly/seasonal drought and 6–10/8–14-day temperature/precipitation polygons, issue/valid metadata, planning-only DOM popups, partial-feed recovery, scene persistence, English/Spanish copy, and headless coverage.
- Added feature-detected App Badging for the foreground saved-view alert watcher, deduplicating active alerts across saved locations, preserving last-good counts, and clearing acknowledged notices without a backend.
- Added bounded PWA file handling for OS-opened GPX/GeoJSON overlays and a File System Access JSON snapshot save-picker with download fallback when unsupported or unavailable.
...
- The camera weather panel now shows the NWS chance of precipitation and a next-12-hour high/low range, clearly labeled as forecast guidance. SW v116.
