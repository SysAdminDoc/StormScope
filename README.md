[![Version](https://img.shields.io/badge/version-0.123.0-blue)](CHANGELOG.md)
...
- **Accessible Hazard & Camera Table** — An on-demand keyboard-readable semantic table exposes active alerts, visible loaded-hazard counts, and nearest verified cameras from data already in memory, with keyboard-reachable alert and camera actions and no extra requests
...
- **METAR Surface Observations** — Optional NOAA Aviation Weather Center station symbols use zoom/viewport-bounded queries, flight-category colors, observed temperature/dewpoint/wind/gust/visibility/ceiling details, raw-report text, freshness, clustering, and last-good recovery
- **Day/Night Orientation** — Optional local solar terminator shading updates in the foreground without network requests and remains below operational overlays
- **NOHRSC Snow Depth** — Optional NOAA NOHRSC snow-depth analysis uses bounded viewport exports, explicit CONUS coverage/freshness/attribution, and remains below operational overlays
...
- **Simple & Pro Layer Detail** — Simple mode keeps the core radar, camera, warning, watch, and wildfire controls focused while retaining status text and enabled NWS alerts; Pro reveals the complete catalog, with the choice stored only on the device
...
- **Opt-in Camera Feed Sanity Check** — A bounded device-local still-frame heuristic can flag likely flat or stalled no-live-feed images without deleting results or changing published provider health; cross-origin pixel access degrades safely to no check
- **Camera Capture Freshness** — Camera list and viewer badges expose stale provider capture timestamps with per-feed-type thresholds and reduced emphasis while unknown timestamps remain explicitly unclassified
- **Device-Local Family Review** — Repeated in-browser camera failures aggregate into a bounded six-hour family review ledger, surfaced in Search health and the viewer without deleting cameras or changing published health
...
- **Live Scene Permalinks** — Public map state debounces into the address bar, and valid hash history entries re-apply without reload while invalid or legacy links preserve the current state
- **Privacy-Bounded Situation Snapshots** — Copy localized plain text or save/download versioned JSON with rounded map coordinates, source issue/freshness, visible-hazard counts, and selected-camera provenance; an optional rounded public scene link excludes private local state
...
- **WPC Winter Storm Severity Index** — Optional, default-off aggregate Days 1–3 impact polygons use the official WPC severity palette and expose issue time, valid period, source attribution, and planning-only limitations in DOM-only popups
...
- **SPC Fire Weather Outlooks** — Optional, default-off Day 1–8 wind/low-humidity and dry-thunderstorm forecast polygons from the keyless NOAA SPC ArcGIS service, with official risk colors, issue/valid times, bounded viewport queries, and DOM-only popups that distinguish forecast guidance from observed NIFC perimeters
...
- **Private Local Overlays** — Strict bounded GeoJSON/GPX imports render with fixed safe styling, remain session-only by default, persist in IndexedDB only after Keep locally, export canonically, and never enter shared scene links or network requests; installed Chromium PWAs can open the same files from the OS file manager or share sheet
- **Local Route Corridors** — Designate an imported LineString, choose a bounded local corridor width, and review intersecting official hazards plus verified cameras ordered along travel; route geometry remains private and can feed a bounded 2–4 camera monitor
- **Private Measurements & Annotations** — Keyboard-accessible point, line, polygon, text, and distance/bearing tools render in a private map pane; drafts stay session-only until explicitly kept locally, and validated exports remain outside scenes and diagnostics
...
- **Observed and Forecast Weather** — US cameras pair the nearest valid NWS station observation, age, source, and distance with independently recoverable hourly forecast context and a clearly separated next-12-hour precipitation guidance timeline; Open-Meteo provides current-condition fallback elsewhere or when both NWS branches fail
...
- [NOAA WPC Winter Storm Severity Index](https://www.wpc.ncep.noaa.gov/wwd/wssi/wssi.php) — Official aggregate Days 1–3 winter-impact polygons (no key)
...
- **Flood, winter, and fire-weather planning context**: NOAA WPC excessive-rainfall/significant-river outlooks, optional WPC Winter Storm Severity Index, optional keyless NOAA NWPS observed/forecast river gauges, and optional SPC Day 1–8 fire-weather forecasts; the UI explicitly distinguishes forecast category from observed stage, wildfire perimeter, and warning claims
- **Fire weather**: [NOAA/NWS SPC fire-weather ArcGIS service](https://mapservices.weather.noaa.gov/vector/rest/services/fire_weather/SPC_firewx/MapServer) and [SPC fire-weather overview](https://www.spc.noaa.gov/products/fire_wx/overview.html), queried by visible viewport and selected outlook day
- **Local overlays**: GeoJSON/GPX files are validated and rendered entirely on-device; file-picker and Web Share Target inputs use the same bounded importer, are never uploaded, linked from properties, or included in scene URLs, and shared-file handoffs are deleted after import
