...
async function waitForApp(page, requireRadar = true) {
  ...
  await page.locator('#camera-count').filter({ hasText: '36,319 cameras' }).waitFor({ state: 'visible' });
  if (requireRadar) {
...
async function main() {
  ...
  try {
    ...
    assert.match(await page.locator('#camera-count').textContent(), /^36\.319 cámaras$/);
    ...
    await page.waitForFunction(() => {
      const results = window._stormscope.getCameraResults();
      return results.length === 29 && results.every((camera) => camera.source === 'usgs');
    });
