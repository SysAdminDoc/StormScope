...
async function waitForApp(page, requireRadar = true) {
  ...
  await page.locator('#camera-count').filter({ hasText: '33,680 cameras' }).waitFor({ state: 'visible' });
  if (requireRadar) {
...
async function main() {
  ...
  try {
    ...
    assert.match(await page.locator('#camera-count').textContent(), /^33\.680 cámaras$/);
