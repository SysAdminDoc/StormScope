...
async function waitForApp(page, requireRadar = true) {
  ...
  await page.locator('#camera-count').filter({ hasText: '24,217 cameras' }).waitFor({ state: 'visible' });
  if (requireRadar) {
...
async function main() {
  ...
  try {
    ...
    assert.match(await page.locator('#camera-count').textContent(), /^24\.217 cámaras$/);
